...
------------------ 따로 추가 안하셔도 됩니다. -------------------
alter table tbl_board
    add (uuid varchar2(100) default 'X');
alter table tbl_board
    add (uploadPath varchar2(200) default 'X');
alter table tbl_board
    add (fileName varchar2(100) default '.no');
alter table tbl_board
    add (filetype char(1) default 'I');
alter table tbl_board
    add (boardUserId varchar2(100) default 'A');
alter table tbl_board
    add (likeNum number default 0); -- 게시물 좋아요 수
alter table tbl_board
    add (replycnt number default 0); -- 게시물 댓글 수
alter table tbl_board
    add (viewNum number default 0);
-- 게시글 조회수
------------------------------------------------------------------
...
select *
from tbl_requestBook; -- tbl_requestBook 테이블 조회
...
---------------------- tbl_requestBook 시퀀스 -------------------------------------
create sequence seq_requestBook;
-- 도서요청 글 번호 시퀀스
-----------------------------------------------------------------------------------
---------------------- tbl_requestBook 제약 조건 ----------------------------------
alter table tbl_requestBook
    add constraint pk_requestBook primary key (r_bookBno);
-----------------------------------------------------------------------------------
alter table tbl_requestBook
    add r_bookProcedure varchar2(50) default '요청됨'
ALTER TABLE tbl_requestBook
    DROP COLUMN r_bookResultMsg;
...
select *
from tbl_requestBookAttach;

---------------------- tbl_requestBookAttach 제약 조건 ---------------------------------
alter table tbl_requestBook
    add constraint pk_requestBook primary key (r_bookBno)
alter table tbl_requestBookAttach
    add constraint fk_requestBook_Attach foreign key (r_bno) references tbl_requestBook (r_bookBno)
----------------------------------------------------------------------------------------------------
...
insert into TBL_BOARD (bno, title, content, category, nickName, boardUserId) (select seq_board.nextval,
                                                                                     title,
                                                                                     content,
                                                                                     category,
                                                                                     nickName,
...
where bno = 10
order by rno asc;
-- bno 10번인 댓글을 rno를 내림차 순으로 정렬


------------------------------------------------------- 잡다한 기능
SELECT *
FROM USER_SEQUENCES; -- 전체 시퀀스 조회
ALTER TABLE tbl_board
    DROP COLUMN fileName; -- 칼럼 삭제
...
select *
from tbl_like -- tbl_like 테이블 전체 조회
...
delete
from TBL_BOARD; -- 더미데이터 삭제

update TBL_BOARD
set category='자유'
where category = '선택'; -- category 칼럼의 '선택' 데이터 값을 '자유'로 변경
